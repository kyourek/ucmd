...
uc_TEST(ucCmdOpt_process_calls_func)
    ...
    ucCmd_set_cmd_tok(cmd, "uart_func\0\n");
...
uc_TEST(ucCmdOpt_format_validation_err_catches_required_arg)
    ...
    ucCmd_set_cmd_tok(cmd, "opt\0\n");
    err = ucCmdOpt_format_validation_err(opt, cmd);
    uc_TRUE(NULL != err);

    ucCmd_set_cmd_tok(cmd, "opt\0arg\0\n");
    err = ucCmdOpt_format_validation_err(opt, cmd);
    uc_TRUE(NULL == err);
...
uc_TEST(ucCmdOpt_format_validation_err_catches_required_switch)
    ...
    ucCmd_set_cmd_tok(cmd, "opt\0-z\0\n");
    err = ucCmdOpt_format_validation_err(opt, cmd);
    uc_TRUE(NULL != err);

    ucCmd_set_cmd_tok(cmd, "opt\0-s\0\n");
    err = ucCmdOpt_format_validation_err(opt, cmd);
    uc_TRUE(NULL == err);
