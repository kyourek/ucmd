...
static ucTestErr ucCmdLineOpt_create_creates_structure(ucTestGroup *p) {
    ...
    ucTEST(NULL != ptr);
    ucTEST(uart_func_one == ucCmdLineOpt_get_func(ptr));
    ucTEST(&state == ucCmdLineOpt_get_state(ptr));
    ucTEST(ucBool_TRUE == ucOpt_is_required((ucOpt*)ptr));

    arg_opt = ucArgOptOwner_get_arg_opt((ucArgOptOwner*)ptr);
    ucTEST(ucOpt_get_name((ucOpt*)arg_opt));
    ucTEST(ucOpt_get_desc((ucOpt*)arg_opt));

    arg_opt = ucArgOpt_get_next(arg_opt);
    ucTEST(ucOpt_get_name((ucOpt*)arg_opt));
    ucTEST(ucOpt_get_desc((ucOpt*)arg_opt));

    ucTEST(NULL == ucArgOpt_get_next(arg_opt));

    switch_opt = ucCmdLineOpt_get_switch_opt(ptr);
    ucTEST(ucOpt_get_name((ucOpt*)switch_opt));
    ucTEST(ucOpt_get_desc((ucOpt*)switch_opt));
...
static ucTestErr ucCmdLineOpt_process_calls_func(ucTestGroup *p) {
    ...
    ucTEST(ucCmdLineOpt_process(cmd_opt, cmd));
...
static ucTestErr ucCmdLineOpt_send_usage_responds_with_usage_string(ucTestGroup *p) {
    ...
    ucTEST(0 == strcmp(expected, transmit_func_one_response));
...
static ucTestErr ucCmdLineOpt_process_handles_invalid_commands(ucTestGroup *p) {
    ...
    ucTEST(0 == strcmp(handle_invalid_command_1_invalid_command, "noopt"));
...
static ucTestErr ucCmdLineOpt_process_does_not_handle_invalid_command(ucTestGroup *p) {
    ...
    err = ucCmdLineOpt_process(opt, cmd);
    ucTEST(0 == strcmp(err, "Invalid command: no option found for \"noopt\""));
